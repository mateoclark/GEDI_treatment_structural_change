...
# input folder with annual s2ndvi images
s2ndviDir <- "E:/active/project/calfire_gedi/treatments/sentinel2_ndvi"
...
# output .csv file
outFile <- paste0("treatment_gedi_s2ndvi_stats_",version,".csv")
#outFile <- paste0("treatment_simulated_gedi_s2ndvi_stats_",version,".csv")
...
pixel_statistics <- function(s2ndvi_raster, polygon){
  extracted_values <- terra::extract(s2ndvi_raster, polygon)
...
# Function for s2ndvi statistics for treatments and associated control areas

s2ndvi_statistics <- function(fgdb, treatment_layer, control_layer, imageFiles,treatmentSelected){
  ...
  for (t in treatmentID){
    ...
    if (EndYear < 2023){
      ...
      # Add to list
      treatments2ndviChange[[length(treatments2ndviChange) + 1]] <- c("s2ndvi",t,TreatmentChangePoly,TreatmentChangePixel,
  ...
  cnames <- c("Metric","TreatmentID",
              "TPolys2ndvimin","TPolys2ndvi1stQ","TPolys2ndvimedian","TPolys2ndvimean","TPolys2ndvi3rdQ","TPolys2ndvimax",
              "TPixels2ndvimin","TPixels2ndvi1stQ","TPixels2ndvimedian","TPixels2ndvimean","TPixels2ndvi3rdQ","TPixels2ndvimax",
              "CPolys2ndvimin","CPolys2ndvi1stQ","CPolys2ndvimedian","CPolys2ndvimean","CPolys2ndvi3rdQ","CPolys2ndvimax",
              "CPixels2ndvimin","CPixels2ndvi1stQ","CPixels2ndvimedian","CPixels2ndvimean","CPixels2ndvi3rdQ","CPixels2ndvimax")
  
  treatmentStats <- data.frame()
  for (i in 1:length(treatments2ndviChange)){
    df <- as.data.frame(t(treatments2ndviChange[[i]]))
...
imageFiles <- list.files(s2ndviDir, pattern = "sentinel2_ndvi", full.names = TRUE)

# Filter images to match years
pattern <- paste(years, collapse = "|")
imageFiles <- imageFiles[grepl(pattern, imageFiles)]

# Process s2ndvi statistics 
treatment_layer <- "treatment_polygons_250119"
control_layer <- "control_polygons_250117"
treatmentStats <- s2ndvi_statistics(fgdb,treatment_layer,control_layer,imageFiles,treatmentSelected)
