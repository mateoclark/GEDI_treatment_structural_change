...
# input folder with annual s2ndmi images
s2ndmiDir <- "E:/active/project/calfire_gedi/treatments/sentinel2_ndmi"
...
# output .csv file
outFile <- paste0("treatment_gedi_s2ndmi_stats_",version,".csv")
#outFile <- paste0("treatment_simulated_gedi_s2ndmi_stats_",version,".csv")
...
pixel_statistics <- function(s2ndmi_raster, polygon){
  extracted_values <- terra::extract(s2ndmi_raster, polygon)
...
# Function for s2ndmi statistics for treatments and associated control areas

s2ndmi_statistics <- function(fgdb, treatment_layer, control_layer, imageFiles,treatmentSelected){
  ...
  for (t in treatmentID){
    ...
    if (EndYear < 2023){
      ...
      # Add to list
      treatments2ndmiChange[[length(treatments2ndmiChange) + 1]] <- c("s2ndmi",t,TreatmentChangePoly,TreatmentChangePixel,
  ...
  cnames <- c("Metric","TreatmentID",
              "TPolys2ndmimin","TPolys2ndmi1stQ","TPolys2ndmimedian","TPolys2ndmimean","TPolys2ndmi3rdQ","TPolys2ndmimax",
              "TPixels2ndmimin","TPixels2ndmi1stQ","TPixels2ndmimedian","TPixels2ndmimean","TPixels2ndmi3rdQ","TPixels2ndmimax",
              "CPolys2ndmimin","CPolys2ndmi1stQ","CPolys2ndmimedian","CPolys2ndmimean","CPolys2ndmi3rdQ","CPolys2ndmimax",
              "CPixels2ndmimin","CPixels2ndmi1stQ","CPixels2ndmimedian","CPixels2ndmimean","CPixels2ndmi3rdQ","CPixels2ndmimax")
  
  treatmentStats <- data.frame()
  for (i in 1:length(treatments2ndmiChange)){
    df <- as.data.frame(t(treatments2ndmiChange[[i]]))
...
imageFiles <- list.files(s2ndmiDir, pattern = "sentinel2_ndmi", full.names = TRUE)

# Filter images to match years
pattern <- paste(years, collapse = "|")
imageFiles <- imageFiles[grepl(pattern, imageFiles)]

# Process s2ndmi statistics 
treatment_layer <- "treatment_polygons_250119"
control_layer <- "control_polygons_250117"
treatmentStats <- s2ndmi_statistics(fgdb,treatment_layer,control_layer,imageFiles,treatmentSelected)
