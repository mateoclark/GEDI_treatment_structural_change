...
# input folder with annual l89ndvi images
l89ndviDir <- "E:/active/project/calfire_gedi/treatments/landsat8_9_ndvi"
...
# output .csv file
outFile <- paste0("treatment_gedi_l89ndvi_stats_",version,".csv")
#outFile <- paste0("treatment_simulated_gedi_l89ndvi_stats_",version,".csv")
...
pixel_statistics <- function(l89ndvi_raster, polygon){
  extracted_values <- terra::extract(l89ndvi_raster, polygon)
...
# Function for l89ndvi statistics for treatments and associated control areas

l89ndvi_statistics <- function(fgdb, treatment_layer, control_layer, imageFiles,treatmentSelected){
  ...
  for (t in treatmentID){
    ...
    if (EndYear < 2023){
      ...
      ## Calculate S2 NDVI change statistics at the polygon and pixel level
      ...
      # S2 NDVI change for treatments using start/end years
      ...
      # Add to list
      treatmentl89ndviChange[[length(treatmentl89ndviChange) + 1]] <- c("l89ndvi",t,TreatmentChangePoly,TreatmentChangePixel,
  ...
  cnames <- c("Metric","TreatmentID",
              "TPolyl89ndvimin","TPolyl89ndvi1stQ","TPolyl89ndvimedian","TPolyl89ndvimean","TPolyl89ndvi3rdQ","TPolyl89ndvimax",
              "TPixell89ndvimin","TPixell89ndvi1stQ","TPixell89ndvimedian","TPixell89ndvimean","TPixell89ndvi3rdQ","TPixell89ndvimax",
              "CPolyl89ndvimin","CPolyl89ndvi1stQ","CPolyl89ndvimedian","CPolyl89ndvimean","CPolyl89ndvi3rdQ","CPolyl89ndvimax",
              "CPixell89ndvimin","CPixell89ndvi1stQ","CPixell89ndvimedian","CPixell89ndvimean","CPixell89ndvi3rdQ","CPixell89ndvimax")
  
  treatmentStats <- data.frame()
  for (i in 1:length(treatmentl89ndviChange)){
    df <- as.data.frame(t(treatmentl89ndviChange[[i]]))
...
imageFiles <- list.files(l89ndviDir, pattern = "landsat8_9_ndvi", full.names = TRUE)

# Filter images to match years
pattern <- paste(years, collapse = "|")
imageFiles <- imageFiles[grepl(pattern, imageFiles)]

# Process l89ndvi statistics 
treatment_layer <- "treatment_polygons_250119"
control_layer <- "control_polygons_250117"
treatmentStats <- l89ndvi_statistics(fgdb,treatment_layer,control_layer,imageFiles,treatmentSelected)
