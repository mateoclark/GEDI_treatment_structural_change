...
# input file
inputFile <- paste0(inDir,"/data/master_gedi_treatment_difference_",version,".csv")
...
processModel <- function(d, metricSelect,outputFile){
  ...
  for (strctVar in metricSelect){
    ...
    dataZscore <- data %>% mutate(Zscore = zscore(data$values)) %>% na.omit()
    ...
    tryCatch({
      aspatial <- nlme::lme(Zscore ~ Type * Timing, data = dataZscore, random = ~ 1|index, control = lmeControl(opt = "optim"))
...
# model all data
outputFile <- paste0(inDir,"/data/nlme_repeated_anova_spatial_zscore_All_",version,".csv")
processModel(d,metricSelect,outputFile)

# model treatment groups 
for (group in unique(d$TreatmentGroup)){
  print(group)
  outputFile <- paste0(inDir,"/data/nlme_repeated_anova_spatial_zscore_",group,"_",version,".csv")
